...
#define portTICK_PERIOD_MS CONFIG_SYSTICK_MS
...
/* Scheduler utilities. */
#define portYIELD() asm volatile("sc" ::: "memory")
...
#define mTaskDefine(task)        \
    TaskHandle_t task##_Handler; \
    portTASK_FUNCTION(task, pvParameters)

#define mTaskCreate(task, name, stk, prio) \
